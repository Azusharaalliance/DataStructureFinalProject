...
RBTPtr MakeTreefromtxt(const char*);
void sim2(RBTPtr, const char*);
void sim(const char*);
...
void main() {
	...
	//	sim("test05.txt");
	//	sim("test06.txt");
	T = MakeTreefromtxt("test01.txt");
	sim2(T, "search01.txt");
	return;
}
RBTPtr MakeTreefromtxt(const char* input) {
	FILE *fp;
	int data;
	NodePtr x;
...
void sim2(RBTPtr T, const char* search) {
	FILE *fp1;
	FILE *fp2;
	int data;
	NodePtr x;
...
void Tree_search1(RBTPtr T, int key)
{
	NodePtr Max;
	NodePtr min;
	NodePtr root;
	NodePtr x = Tree_search(T, T->root, key);
	Max = T->nil;
	min = T->nil;
	//tree에서 key보다 작은 수들 중에서 최대인 값
	root = T->root;
	while (root != T->nil) {
		if (key > root->val) {
			Max = root;
